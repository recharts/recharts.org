export default {
  ...
  any string or number
      ...
      isOptional: true,
      desc: {
        'en-US': 'If set a React element, the option is the custom react element of rendering label.  If set a function, the function will be called to render label content.',
        'zh-CN': '定制 Label 展示的内容。如果值为 React element，会克隆这个元素来渲染 Label 的内容。如果值为函数，会调用这个函数来生成 Label 的内容。',
      },
      format: [
        `<Label content={<CustomizedTooltip external={external} />} />`,
        `<Label content={renderTooltip} />`,
      ],
      // examples: [
      //   {
